...
const RecentsPage: React.FC = () => {
    ...
    const removeCall = (id: number) =>
        SetState({
            ...state,
            calls: state.calls.filter(call => call.id != id),
        });
    const removeAllCalls = () =>
        SetState({ ...state, deleteMode: false, calls: [] });
    const toggleOnlyMissed = () =>
        SetState({ ...state, onlyMissed: !state.onlyMissed });
    const toggleDeleteMode = () =>
        SetState({ ...state, deleteMode: !state.deleteMode });
        ...
        state.calls
            ...
            .map(call => (
                <Call
                    ...
                    onCallDeleted={() => removeCall(call.id)}
                />
            ));
        ...
        <div>
            <div className="header">
                <div className="header-left">
                    {state.deleteMode && (
                        <div onClick={removeAllCalls}>Clear</div>
                    )}
                </div>
                <div className="header-middle call-filter">
                    <div
                        className={`call-filter-btn ${
                            state.onlyMissed ? "" : "active"
                        }`}
                        onClick={toggleOnlyMissed}
                    >
                        All
                    </div>
                    <div
                ...
                <div className="header-right" onClick={toggleDeleteMode}>
                    {state.deleteMode ? "Done" : "Edit"}
                </div>
            </div>
...
const Call: React.FC<ICallProps> = (props: ICallProps) => {
    ...
    return (
        <li
            className={props.deleteMode && deleting ? "deleting" : ""}
            onClick={() => deleting && setDeleting(false)}
        >
            {props.deleteMode && (
                <div onClick={() => setDeleting(true)}>
                    <ReactSVG className="minus-icon" src={MinusIcon}></ReactSVG>
                </div>
            )}
            <div className="left-icon-container">
                {props.outgoing && (
                    <ReactSVG src={OutgoingIcon} className="outgoing-icon" />
                )}
            </div>
            ...
            <div
                className={`confirm-delete-btn ${props.deleteMode &&
                    deleting &&
                    "delete-btn-displayed"}`}
