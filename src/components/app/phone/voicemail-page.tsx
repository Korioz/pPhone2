...
const VoicemailPage: React.FC = () => {
    ...
    const toggleVoicemailSelected = (id: number) =>
        SetState({
            ...state,
            voicemails: state.voicemails.map(mail =>
                mail.id == id ? { ...mail, selected: !mail.selected } : mail
            ),
        });

    const selectedItems = state.voicemails.filter(mail => mail.selected);
    const hasSelectedItems = selectedItems.length > 0;
    const hasUnreadSelectedItems =
        selectedItems.filter(mail => !mail.read).length > 0;
    ...
    const getVoiceMails = () => {
        return state.voicemails.map(mail => (
            <Voicemail
                ...
                editMode={state.editMode}
                onVoicemailClicked={() =>
                    state.editMode && toggleVoicemailSelected(mail.id)
                }
            />
        ));
    };
    ...
    return (
        <div>
            <div className="header">
                <div className="header-left">Greeting</div>
                <div className="header-middle">Voicemail</div>
                <div className="header-right" onClick={toggleEditMode}>
            ...
            {state.editMode && (
                <div id="voicemail-options">
                    <div
                        ...
                        className={hasUnreadSelectedItems ? "enable" : ""}
                    >
                        Mark as read
                    </div>
                    <div
                        onClick={deleteSelected}
                        className={hasSelectedItems ? "enable" : ""}
                    >
                        Delete
                    </div>
                </div>
    ...
    return (
        ...
        >
            {props.editMode && !props.selected && (
                <ReactSVG className="circle-icon" src={CircleEmptyIcon} />
            )}
            {props.editMode && props.selected && (
                <ReactSVG
