...
const Calculator: React.FC = () => {
    ...
    const numberClicked = (n: number) => {
        ...
        if (state.input && digitsCount(state.input) > 8) return;
    ...
    const operationClicked = (operation: Operation) => {
        let newState =
            state.operation != undefined
                ...
                : {
                      ...state,
                      result: state.input ? state.input : state.result,
                      input: undefined,
                      decimal: false,
                  };
    ...
    const getResultState = () => {
        let newState = { ...state };
        const value =
            newState.input != undefined ? newState.input : newState.lastInput;
        const operation =
    ...
    return (
        <AppContainer backgroundColor="#000000">
            <div id="calculator">
                <CalculatorInput
                    input={state.input ? state.input : state.result}
                />
                <div className="btn-container">
                    ...
                    <div
                        className={`btn-circle ${
                            state.operation == Operation.Divison &&
                            state.input == undefined
                                ? "btn-white"
                                : "btn-orange"
                        }`}
                    ...
                    <div
                        className={`btn-circle ${
                            state.operation == Operation.Multiplication &&
                            state.input == undefined
                                ? "btn-white"
                                : "btn-orange"
                        }`}
                        onClick={() =>
                            operationClicked(Operation.Multiplication)
                        }
                    >
                        &times;
                    </div>
                    ...
                    <div
                        className={`btn-circle ${
                            state.operation == Operation.Substraction &&
                            state.input == undefined
                                ? "btn-white"
                                : "btn-orange"
                        }`}
                    ...
                    <div
                        className={`btn-circle ${
                            state.operation == Operation.Addition &&
                            state.input == undefined
                                ? "btn-white"
                                : "btn-orange"
                        }`}
...
const CalculatorInput: React.FC<CalculatorInputProps> = props => {
    ...
    useEffect(() => {
        if (inputRef.current) {
            let size = 65;
            while (
                getTextWidth(formated, size) > inputRef.current.clientWidth &&
                size > 0
            ) {
                size -= 0.5;
            }
    ...
    return <h1 className="calculator-result" ref={inputRef}></h1>;
};
